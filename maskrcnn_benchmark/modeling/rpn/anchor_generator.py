...
class AnchorGenerator(nn.Module):
    ...
    def __init__(
        ...
        if len(anchor_strides) == 1:
            ...
            # for FPN, each feature map get one anchor_stride and one size, generate_anchors() return [len(aspect_ratios), 4]
            cell_anchors = [
                generate_anchors(
                    anchor_stride,
...
def _generate_anchors(base_size, scales, aspect_ratios):
    ...
    # reshape anchor to anchors follow aspect_ratios [len(aspect_ratios), 4]
    anchors = _ratio_enum(anchor, aspect_ratios)
    # resize anchors to enlarge scales [len(aspect_ratios), 4]
    anchors = np.vstack(
